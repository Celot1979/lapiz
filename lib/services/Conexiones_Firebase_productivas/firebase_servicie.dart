...
class FirebaseService {
  ...
  Future<List> getNota(String tipo) async {
    ...
    try {
      ...
      if (snapshot.docs.isEmpty) {
        ...
        print(
            'La colección $coleccion no existe.'); // Aquí puedes reemplazar con un método para mostrar un mensaje emergente
        return [];
      }
  ...
  Future<void> addNota(Map<String, dynamic> nota) async {
    try {
      String coleccion = nota[
          'clasificacion']; // Obtener el nombre de la colección del campo 'clasificacion'
  ...
  Future<void> crearColeccionVacia(String nombreColeccion) async {
    try {
      // Crear un documento temporal en la nueva colección
      await _firestore.collection('categorias').doc(nombreColeccion).set({
        'nombre': nombreColeccion,
        'createdAt': FieldValue.serverTimestamp(),
        'tipo': 'personalizada'
      });

      print('Colección "$nombreColeccion" creada exitosamente en Firebase');
    } catch (e) {
      print('Error al crear la colección en Firebase: $e');
      throw Exception('No se pudo crear la colección en Firebase: $e');
    }
  }

  // Nuevo método para obtener todas las colecciones

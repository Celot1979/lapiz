...
class _NotasState extends State<Notas> {
  ...
  Future<void> _agregarNuevaCategoria() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => const CrearCategorias()),
    );
...
// Nueva clase para mostrar las notas de una categoría personalizada
...
class _NotasCategoriaPageState extends State<NotasCategoriaPage> {
  ...
  Future<void> _cargarNotas() async {
    final prefs = await SharedPreferences.getInstance();
    final notasString = prefs.getString('notas_${widget.categoria}') ?? '[]';
    setState(() {
      notas = List<Map<String, dynamic>>.from(json.decode(notasString));
    });
  }

  Future<void> _guardarNota(String texto) async {
    final prefs = await SharedPreferences.getInstance();
    final nota = {
      'texto': texto,
    ...
    notas.add(nota);
    await prefs.setString('notas_${widget.categoria}', json.encode(notas));
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Notas - ${widget.categoria}'),
      ),
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            ...
            subtitle: Text(DateTime.parse(nota['fecha']).toString()),
          );
        },
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          // Aquí puedes implementar la creación de nuevas notas
